    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          ...
          <a href="<?= base_url() ?>chat/office_list" class="nav-link pt-4 pl-1"><i class="fas fa-chevron-left"></i> チャット一覧</a>
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div><!-- /.content-header -->
    <!-- Main content -->
      ...
      <div class="container-fluid">
        ...
        <div class="card card-info direct-chat direct-chat-primary">
          <div class="card-header">
            ...
            <h3 class="card-title">【<i class="fas fa-users"></i> <?= $you_array['office'] ?> / <i class="fas fa-user"></i> <?= $you_array['name'] ?>】</h3>
          </div><!-- /.card-header -->
          <div class="card-body">
            ...
            <div class="direct-chat-messages" id="chat_message">
              ...
              foreach ($array_reverse  as $value) {
                if ($value['delete_chat'] === "0") { ?>
                  ...
                  <?php if ($value['myworker_id'] === $my_array['id'] && $value['youoffice_id'] === $you_array['id']) { ?>
                    <div class="direct-chat-msg">
                      <div class="direct-chat-infos clearfix">
                        <span class="direct-chat-name float-left"><?= $my_array['name'] ?></span>
                        <span class="direct-chat-timestamp float-right"><?= $value['insert_time'] ?></span>
                      </div><!-- /.direct-chat-infos -->
                      <img class="direct-chat-img" src="<?= base_url() ?>assets/images/resize_work/<?= $my_array['img'] ?>" alt="team1"> <!-- /.direct-chat-img -->
                      <div class="direct-chat-text float-left">
                        <?= $value['message'] ?>
                      </div><!-- /.direct-chat-text -->
                    </div><!-- /.direct-chat-msg -->
                  <?php  } ?>
                  <?php if ($value['myoffice_id'] === $you_array['id'] && $value['youworker_id'] === $my_array['id']) { ?>
                    <!-- Message to the right -->
                    <div class="direct-chat-msg right">
                      <div class="direct-chat-infos clearfix">
                        <span class="direct-chat-name float-right"><?= $you_array['name'] ?></span>
                        <span class="direct-chat-timestamp float-left"><?= $value['insert_time'] ?></span>
                      </div><!-- /.direct-chat-infos -->
                      <img class="direct-chat-img" src="<?= base_url() ?>assets/images/resize_office/<?= $you_array['img'] ?>" alt="team2"> <!-- /.direct-chat-img -->
          ...
          <div class="card-footer">
            ...
            <div class="input-group">
              <input type="hidden" class="form-control" name="myworker_id" value="<?= $my_array['id'] ?>">
              <input type="hidden" class="form-control" name="youoffice_id" value="<?= $you_array['id'] ?>">
